...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        if(start == null || end == null) throw new NoSuchElementException("Cannot "
            + "find a shortest path with null start and end points"); 
        Vertex sourceVertex = vertices.get(start);
        Vertex targetVertex = vertices.get(end);
        if(sourceVertex == null || targetVertex == null) throw new 
        NoSuchElementException("Cannot find a shortest path between two "
            + "vertices that do not exist");
        
        //Data Fields for the Implementation
        Hashtable<Vertex, Path> pathsThatHaveBeenFound = new Hashtable<Vertex, Path>();
        PriorityQueue<Path> frontier = new PriorityQueue<Path>();
        Hashtable<Vertex, Path> visited = new Hashtable<Vertex, Path>();
        
        //Add the Source Vertex to the Priority Queue
        Path sourcePath = new Path(sourceVertex);
        frontier.add(sourcePath);
        
        //Once the Priority Queue is Empty, we are done iterating
        while(!frontier.isEmpty()) {
            Path current = frontier.poll();
            visited.put(current.end, current);
            for (Edge e : current.end.edgesLeaving) {
                Path copy = new Path(current);
                copy.extend(e);
                if (!pathsThatHaveBeenFound.containsKey(e.target)) {
                    pathsThatHaveBeenFound.put(e.target, copy);
                } else if (copy.compareTo(pathsThatHaveBeenFound.get(e.target)) < 0) {                    
                    pathsThatHaveBeenFound.replace(e.target, copy);
                }
        ...
        //If the Vertex isn't contained in Visited, then it was impossible to 
        //reach
        if(!visited.containsKey(targetVertex)) {
            throw new NoSuchElementException();
        }
        
        //Otherwise return the Path for the TargetVertex
        return pathsThatHaveBeenFound.get(targetVertex);   
    }
